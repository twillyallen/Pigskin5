...
// Returns the Sunday that starts the Sun–Sat week containing dateStr
...
Deno.serve(async (_req) => {
  ...
  const sunday = weekStartStr(saturday); // Sun–Sat week: same as the leaderboard
  ...
  const dateRange = `${fmtShort(sunday)}–${fmtShort(saturday)}`;

  const { data: attempts, error } = await supabase
    .from("quiz_attempts")
    .select("user_id, points, quiz_date, submitted_at, profiles!inner(username, twitter_handle)")
    .in("quiz_date", dates);
  ...
  // Sum points per user; track days played and earliest last-submission for tiebreaking
  ...
  for (const a of attempts) {
    ...
    totals[uid].daysPlayed.add(a.quiz_date);
  ...
  const daysPlayed = winner.daysPlayed.size;

  const tweet =
    `#Pigskin5 Weekly Winner (${dateRange}):\n\n` +
    `${mention}!!\n` +
    `${winner.points.toLocaleString()} pts over ${daysPlayed} day${daysPlayed === 1 ? "" : "s"}\n` +
    `Built different.\n` +
    `New week, new chance.`;
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ winner: winner.username, points: winner.points, dateRange }),
      { status: 200 },
    );
  } catch (err) {
